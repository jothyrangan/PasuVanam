...
export const Header = () => {
    ...
    const { totalItems } = useCart(); // Added useCart hook
        ...
        >
            <Container>
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <Link href={ROUTES.CART} className="relative p-2 text-gray-700 hover:text-primary transition-colors">
                            ...
                            <nav className="flex flex-col gap-4">
                                {NAV_LINKS.map((link) => (
                                    <Link
                                        key={link.name}
                                        href={link.href}
                                        className={`block py-3 text-lg font-medium border-b border-gray-100 ${pathname === link.href ? "text-primary" : "text-gray-800"
                                            }`}
                                    >
                                        {link.name}
                                    </Link>
